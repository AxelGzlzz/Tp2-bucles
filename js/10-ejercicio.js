/*
10- Realiza un script que pida número de filas y columnas y escriba una tabla.
Dentro de cada una de las celdas deberá escribirse un número consecutivo en
orden descendente. Si, por ejemplo, la tabla es de 7×5 los números irán del 35 al 1
*/

//usar prompt y pedir filas y columnas

const fila = parseInt(prompt("ingrese un numero de fila"));
const col = parseInt(prompt("ingrese un numero de columnas"));
let celda = fila * col;

console.log(fila);
console.log(col);

document.write(`<table><tbody>`);
for (let indfila = 1; indfila <= fila; indfila++) {
  document.write(`<tr>`);
  for (let indcolumna = 1; indcolumna <= col; indcolumna++) {
    document.write(`<td>${celda--}</td>`);
  }

  document.write(`</tr>`);
}

document.write(`</tbody></table>`);
